        ...
        */
        ...
        async function applyPresetCharacter(characterName) {
            ...
            if (modelParts.torso) {
                ...
                if (characterName === 'Baby') {
                    torsoColor = new THREE.Color('#FE50C6'); // Rosa intenso para Baby
                } else if (characterName === 'Mystery') {
                    torsoColor = new THREE.Color('#8166F1'); // Morado para Mystery
                } else if (characterName === 'Romance') {
                    torsoColor = new THREE.Color('#FCEE35'); // Color torso para Romance (amarillo)
                } else if (characterName === 'Abby') {
                    torsoColor = new THREE.Color('#A8F4E4'); // Color torso para Abby
                } else if (characterName === 'Jinu') {
                    torsoColor = new THREE.Color('#F4F4F2'); // Gris claro para Jinu
                } else if (characterName === 'Mira') {
                    torsoColor = new THREE.Color('#243267');
                } else if (characterName === 'Rumi') {
                    torsoColor = new THREE.Color('#FAB823');
                } else if (characterName === 'Steve') {
                    torsoColor = new THREE.Color('#00A9AF'); // Camisa cian de Steve
                }

                modelParts.torso.traverse((child) => {
                    if (child.isMesh && child.material && child.material.color) {
                        // Excluir meshes de ropa
            ...
            // Aplicar color a los brazos
            // Colores especiales para brazos
            let armsColor = lightestSkinColor;
            if (characterName === 'Baby') {
                armsColor = new THREE.Color('#FCD5C6'); // Color piel para Baby
            } else if (characterName === 'Mystery') {
                armsColor = new THREE.Color('#FCD5C6'); // Color piel para Mystery
            } else if (characterName === 'Romance') {
                armsColor = new THREE.Color('#FCD5C6'); // Color piel para Romance
            } else if (characterName === 'Abby') {
                armsColor = new THREE.Color('#FCD5C6'); // Color piel para Abby
            } else if (characterName === 'Jinu') {
                armsColor = new THREE.Color('#FCD5C6'); // Color piel para Jinu
            } else if (characterName === 'Steve') {
                armsColor = new THREE.Color('#AF7E5C'); // Piel de Steve
            }
            ...
            // Colores especiales para piernas
            let legsColor = lightestSkinColor;
            if (characterName === 'Baby') {
                legsColor = new THREE.Color('#6A68E9'); // Azul violeta para Baby
            } else if (characterName === 'Mystery') {
                legsColor = new THREE.Color('#91C9FA'); // Azul claro para Mystery
            } else if (characterName === 'Romance') {
                legsColor = new THREE.Color('#DEFEFF'); // Azul muy claro para Romance
            } else if (characterName === 'Abby') {
                legsColor = new THREE.Color('#91C9FA'); // Azul claro para Abby
            } else if (characterName === 'Jinu') {
                legsColor = new THREE.Color('#91C9FA'); // Azul claro para Jinu
            } else if (characterName === 'Mira') {
                legsColor = new THREE.Color('#390626');
            } else if (characterName === 'Rumi') {
                legsColor = new THREE.Color('#292827');
            } else if (characterName === 'Zoey') {
                legsColor = new THREE.Color('#16275C');
            } else if (characterName === 'Steve') {
                legsColor = new THREE.Color('#4332A0'); // Pantalones azules de Steve
            }
            ...
            // Buscar todas las texturas del personaje
            let eyeTexture = eyeTextures.find(t => t.name === characterName);
            let eyebrowTexture = eyebrowTextures.find(t => t.name === characterName);
            let earTexture = earTextures.find(t => t.name === characterName);
            const noseTexture = (characterName === 'Steve') ? noseTextures.find(t => t.name === 'Steve') : noseTextures.find(t => t.name === 'Default');
            const hairTexture = hairTextures.find(t => t.name === characterName);
            const torsoTexture = torsoClothingTextures.find(t => t.name === characterName);
            const armTexture = armTextures.find(t => t.name === characterName);
            const legTexture = legTextures.find(t => t.name === characterName);

            // Casos especiales de fallback para Steve (por si falla la búsqueda en character-data.js)
            if (characterName === 'Alex') {
                ...
                // Enderman NO TIENE BRAZOS NI PIERNAS (según repo)
                armTexture = null;
                legTexture = null;
            }
        }
